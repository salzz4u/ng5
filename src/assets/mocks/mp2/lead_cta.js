...
var oabNoticesUrl = "/mes/messagesImportantNotices/null/messagesImportantNoticesDetails?mode=form&LaunchOverlayType=OAB";
...
var plocOfferUrl = "/mes/messagesBMOOffers/null/messagesBMOOffersDetails?mode=form&showAirMilesYorN=N&bmooffer=Y&LaunchOverlayType=PAPLOC";
...
var switchPlanUrl = "/mes/messagesImportantNotices/null/preApprovedOffers?mode=confirmation&trackForm=Y&productType=ImportantNotice.PAO&offerAction=A&leadCode=Campaign ID&pageId=123";
...
var interstitialOABFrame="/OLBmain/ssoOai?mode=confirmation&olb=y&oai_jump_app=OAB&oai_jump_entry=manage&frameId=interstitialFrame";
...
function evaluateAnchor(event) { //message centre URLs
    var obj;
...
//            obj.onClick = interstitialNotifyOLBmain(obj, LogFulfilledSales, "/sre");
        ...
        case objClass.testClass("MortgageRenewal5Fixed") :
            obj.onClick = notifyOlbOffersForm(obj, LogFulfilledSales, preapproveMortgage, 'A', 'MGRN', {'rateType': '5 Year Fixed'});
            break;
        ...
        case objClass.testClass("MortgageRenewal5Variable") :
            obj.onClick = notifyOlbOffersForm(obj, LogFulfilledSales, preapproveMortgage, 'A', 'MGRN', {'rateType': '5 Year Variable'});
            break;
        ...
        case objClass.testClass("checkboxCC_PAPLOC_AirMiles") :
            obj.target = "_top";
            obj.onClick = continueIfAgreed(CC_preapprovedPlocUrl + '&showAirMilesYorN=Y&status=1','MC','PAPLOC','A',olbPath,LogFulfilledSales);
            break;
        case objClass.testClass("checkboxCC_PAPLOC") :
            obj.target = "_top";
            obj.onClick = continueIfAgreed(CC_preapprovedPlocUrl + '&showAirMilesYorN=N&status=1','MC','PAPLOC','A',olbPath,LogFulfilledSales);
            break;
...
function notifyOlb(obj,trackingcode,url) { // with olbPath
...
function notify(obj,trackingcode,url) { // no olbPath
    if(inprint != undefined && ( inprint == 'Y' || inprint == 'y')) {
        return false;
    }
...
function notifyNoInprint(obj,trackingcode,url) { // no inprint
...
function notifyOlbOffers(obj, trackingcode, url, action, productType) {
    ...
    if(magicId != undefined) {
        ...
        olbUrl = olbUrl + '&trackingCode=' + trackingcode + '&magicId=' + magicId + '&productType=' + productType + '&offerAction=' + action;
    ...
    if(magicId != undefined) {
        ...
        if (userSelectionJson) {
            for (var k in userSelectionJson) {
                if (typeof k == "string")
                    _createFormElement("extra(" + k + ")", userSelectionJson[k], tForm, "hidden");
            }
        }
...
var emptyMsg = "";
var invalidMsg = "";
var emailRE = new RegExp("^\\w+((-\\w+)|(\\.\\w+)|(\\_\\w+)|(\\'\\w+)|(-))*\\@[A-Za-z0-9]+((\\.|-|_)[A-Za-z0-9]+)*\\.[A-Za-z0-9]+$");
var emptyEmailEN = "Email Address is required.";
var emptyEmailFR = "Courriel est requise.";
var emptyInvalidEmailEN = "The email address is invalid.  Please ensure that only one '@' symbol is in the address.  Also, ensure that you include the '.' in the address, and at least one character should be on either side of the '@' and the '.'";
var emptyInvalidEmailFR = "L'adresse &eacute;lectronique n'est pas valide. Veuillez vous assurer que le << @ >> n'apparant qu'une seule fois dans l'adresse. Assurez-vous &eacute;galement d'inclure le << . >> dans l'adresse. De plus, au moins un caract&egrave;re devrait se situer de part et d'autre du << @>> et du << . >>.";
/*   test data - TODO remove when implement in production
*/
var trace_enabled = true;

var isPreviewMode = false;
var test_offerAdCB = '{"marketingOfferAdCB": {'
...
var test_enroll_success = '{'
                     + '"status":"success"'
                     + ',"referenceNumber":"0001235"'
					 + '}';

var test_enroll_failed = '{'
                     + '"status":"failure"'
                     + ',"errorCode":"E010011"'
					 + '}';

var test_subAlert_success = '{'
                     + '"status":"success"'
					 + '}';

var test_subAlert_failed = '{'
                     + '"status":"failure"'
                     + ',"errorCode":"E010021"'
					 + '}';
/*
        ...
        try {
            if (xmlhttp.readyState == 4) {
                ...
                if (xmlhttp.status == 200) {
                    ...
                    if (enrollJson &&
                        (typeof enrollJson.Status != 'undefined') &&
                        enrollJson.Status
                    ) {
                        ...
                        if (enrollJson.Status === success) {
                            ...
                            if ((typeof enrollJson.ErrorCode != 'undefined') && enrollJson.ErrorCode) {
                                ...
                                sendError2(generalErrorDiv, "generalError_errorCode", "2020 : enrollEStatement error = " + +enrollJson.ErrorCode);
                            } else {
...
function notifyOlbInterstitial(obj,trackingcode,url) { // with olbPath + explicit LME tracking
    if(inprint != undefined && ( inprint == 'Y' || inprint == 'y')) {
        return false;
    }
    var olbUrl = 'https://' + domain + olbPath + url ;
...
function interstitialNotifyOLBmainTarget(obj, _lmeTrackingCode, url ) { // with no olbPath + explicit LME tracking + don't override target
    if(inprint != undefined && ( inprint == 'Y' || inprint == 'y')) {
        return false;
    }
...
function interstitialNotifyOLBmain2(obj,_lmeTrackingCode,url) { // with no olbPath + explicit LME tracking + magicid + don't override target
    if(magicId != undefined) {
...
function interstitialNotifyOLBmain1(obj,_lmeTrackingCode,url) { // with olbPath + explicit LME tracking + magicid
    if(magicId != undefined) {
        url = url + '&magicId=' + magicId;
    }
...
function interstitialNotifyOlb(obj,_lmeTrackingCode,url) { // with olbPath + explicit LME tracking

    return interstitialNotifyOLBmain(obj, _lmeTrackingCode, olbPath + url );
}

function interstitialNotifyOLBmain(obj, _lmeTrackingCode, url ) { // with no olbPath + explicit LME tracking
    if(inprint != undefined && ( inprint == 'Y' || inprint == 'y')) {
        return false;
    }
    sendLMETrackingCode(_lmeTrackingCode);
...
function subAlert(nextDiv, email_addrDiv, errorDiv) {
    ...
    if (!email_addr) {
        ...
        if (!email_addr.match(emailRE)) {
            ...
            var url = "https://" + domain + subscribeEstatementURL + "?magicId=" + magicId + "&" + "email=" + encodeURIComponent(email_addr);
            xmlhttp.open("POST", url, true);
                ...
                try {
                    if (xmlhttp.readyState == 4) {
                        ...
                        if (xmlhttp.status == 200) {
                            ...
                            if (
                            subAlertJson &&
                            (typeof subAlertJson.Status != 'undefined') &&
                            subAlertJson.Status
                            ) {
                                if (subAlertJson.Status === success) {
                                    ...
                                    if (
                                    (typeof subAlertJson.ErrorCode != 'undefined') &&
                                    subAlertJson.ErrorCode
                                ) {
...
function sendError2(errorDiv, errorMsgDiv, errorCode) {
    ...
    if (document.getElementById(errorMsgDiv))
        document.getElementById(errorMsgDiv).innerHTML = errorCode;
    showDiv(errorDiv);
...
function interstitialOAB( ) {
    ...
    showOaiDialog(this, 'OVERLAY', '/onlinebanking/OLBmain/ssoOai?mode=confirmation&olb=y&oai_jump_app=OAB&oai_jump_entry=new', {width:1000, height:750}, 'OABFrame', 'OABDialog');
    return false;
}
